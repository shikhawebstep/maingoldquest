    ...
    const handleInputChange = useCallback((e, input, index) => {
        ...
        setServicesDataInfo((prev) => {
            ...
            if (reportFormJson.heading === "INSTA DRUG TEST") {
                ...
                const drugPanel = annexureData["drug_panel_insta_drug_test"];
                            ...
                            <div className="SelectedServices border md:p-5 p-2 overflow-auto rounded-md md:mx-12">
                                ...
                                {servicesDataInfo && servicesDataInfo.map((serviceData, index) => {
                                    if (serviceData.serviceStatus) {
                                        const formJson = JSON.parse(serviceData.reportFormJson.json);
                                        const dbTable = formJson.db_table;
                                        let status = serviceData?.annexureData?.status; // Default to "initiated"
                                        let sortingOrderfinal = serviceData?.annexureData?.sorting_order || "";
                                        let preselectedStatus = selectedStatuses[index] || status || (selectedStatuses[index] || "initiated");
                                        let preselectedSortingOrder = sortingOrder[dbTable] ?? sortingOrderfinal;
                                        // Condition to show "INSTA DRUG TEST" services only when the checkbox is checked or the heading is "INSTA DRUG TEST"
                                        const showInstaDrugTest = formData.updated_json.insta_drug_test && formJson.heading === "INSTA DRUG TEST";

                                        if (showInstaDrugTest || !formData.updated_json.insta_drug_test) {
                                            return (
                                    ...
                                    if (serviceData.serviceStatus) {
                                        ...
                                        return (
                                            <div key={index} className="mb-6 overflow-x-auto">
                                                ...
                                                {(showInstaDrugTest || !formData.updated_json.insta_drug_test) && selectedStatuses[index] !== "nil" && (
                                                    <>
                                                        {dbTableHeading && (
